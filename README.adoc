...
(def-action-db
 :my-db-action ;; <1>
 [:my-co-effect-to-inject] ;; <2>
 (fn [db] ;; <3>
   (assoc db :key :value)))
----
<1> - Machine unique action id
<2> - List of co-effects to inject into re-frame's co-effects map
<3> - Action handler

==== Entry / Exit actions

When machine enters to or exits from a state it might execute a set of actions. The actions are defined with following macros
